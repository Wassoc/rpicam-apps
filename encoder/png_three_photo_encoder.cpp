...
/*
 * png_three_photo_encoder.cpp - PNG encoder that combines three
...
#include "png_three_photo_encoder.hpp"
...
// Helper function to create EXIF entry (same as other encoders)
...
// Helper function to set EXIF string (same as other encoders)
...
// Create EXIF data from metadata using libexif (largely copied from png_encoder)
	...
	: Encoder(options), abortEncode_(false), abortOutput_(false), index_(0), output_index_(0)
{
	...
	for (int i = 0; i < NUM_ENC_THREADS; i++)
		encode_thread_[i] = std::thread(std::bind(&PngThreePhotoEncoder::encodeThread, this, i));
	LOG(2, "Opened PngThreePhotoEncoder");
}

PngThreePhotoEncoder::~PngThreePhotoEncoder()
{
	abortEncode_ = true;
	for (int i = 0; i < NUM_ENC_THREADS; i++)
	...
	abortOutput_ = true;
	output_thread_.join();
	LOG(2, "PngThreePhotoEncoder closed");
}

void PngThreePhotoEncoder::EncodeBuffer(int fd, size_t size, void *mem, StreamInfo const &info,
										int64_t timestamp_us, Metadata const &post_process_metadata,
										libcamera::ControlList const &control_list_metadata)
{
	(void)fd;
	(void)size;
	(void)control_list_metadata;

	std::lock_guard<std::mutex> lock(encode_mutex_);
	MonoFrameItem item = { mem, info, timestamp_us, index_++, post_process_metadata };
	encode_queue_.push(item);
	encode_cond_var_.notify_all();
}

void PngThreePhotoEncoder::encodePNG(MonoFrameItem const &red,
									 ...
									 size_t &buffer_len)
{
	...
	try
	{
		// Basic sanity checks: all frames must match in geometry.
		if (red.info.width != green.info.width || red.info.width != blue.info.width ||
			red.info.height != green.info.height || red.info.height != blue.info.height)
			throw std::runtime_error("RGB triplet frames have mismatched dimensions");

		// Initialize memory buffer
		mem_buffer.capacity = red.info.width * red.info.height * 3 + 1024; // Initial estimate
		...
		// Set image attributes: 8-bit RGB
		png_set_IHDR(png_ptr, info_ptr, red.info.width, red.info.height, 8, PNG_COLOR_TYPE_RGB,
		...
		// Add EXIF metadata as PNG eXIf chunk (similar to PngEncoder)
		// For a composite RGB image, override lamp color description.
		Metadata exif_metadata = red.metadata;
		try
		{
			exif_metadata.Set("exif_data.lamp_color", std::string("RGB composite"));
		}
		catch (...)
		{
			// If Set throws or is unavailable, just continue with original metadata.
		}

		std::string temp_lamp_color;
		if (exif_metadata.Get(std::string("exif_data.lamp_color"), temp_lamp_color) == 0)
		{
			try
			{
				// Create EXIF data using libexif
				uint8_t *exif_buffer = nullptr;
				unsigned int exif_len = 0;
				create_exif_data(exif_metadata, exif_buffer, exif_len);
				if (exif_buffer && exif_len > 0)
				{
					// libexif returns TIFF data; PNG eXIf chunk requires "Exif\0\0" prefix
					bool needs_prefix = (exif_len < 6 || memcmp(exif_buffer, "Exif", 4) != 0);
					if (needs_prefix)
					{
						exif_data_storage.resize(6 + exif_len);
						memcpy(exif_data_storage.data(), "Exif\0\0", 6);
						memcpy(exif_data_storage.data() + 6, exif_buffer, exif_len);
						free(exif_buffer);
					}
					else
					{
						exif_data_storage.assign(exif_buffer, exif_buffer + exif_len);
						free(exif_buffer);
					}
					...
					LOG(2, "Added EXIF data to RGB PNG, size: " << exif_data_storage.size());
				}
			}
			catch (std::exception const &e)
			{
				LOG_ERROR("Failed to create EXIF data for RGB PNG: " << e.what());
				// Continue without EXIF
			}
		}

		// Interleave three monochrome planes into RGB888 buffer.
		const unsigned int width = red.info.width;
		const unsigned int height = red.info.height;
		const unsigned int stride = red.info.stride;

		std::vector<uint8_t> rgb_data(width * height * 3);

		for (unsigned int y = 0; y < height; ++y)
		{
			const uint8_t *r_row = (uint8_t *)red.mem + y * stride;
			const uint8_t *g_row = (uint8_t *)green.mem + y * stride;
			const uint8_t *b_row = (uint8_t *)blue.mem + y * stride;
			uint8_t *rgb_row = rgb_data.data() + y * width * 3;
		...
		// Set up the image data for libpng
		png_byte **row_ptrs = (png_byte **)png_malloc(png_ptr, height * sizeof(png_byte *));
		for (unsigned int y = 0; y < height; ++y)
			row_ptrs[y] = rgb_data.data() + y * width * 3;
...
void PngThreePhotoEncoder::encodeThread(int num)
{
	(void)num;
	...
	MonoFrameItem encode_item;
	std::vector<MonoFrameItem> group;
	group.reserve(3);
	...
	{
		{
			...
			while (true)
			{
				...
				{
					if (frames)
						LOG(2, "PngThreePhotoEncoder encoded " << frames << " RGB frames, average time "
		...
		// Accumulate frames into groups of three (R, G, B).
		group.push_back(encode_item);
		if (group.size() < 3)
			continue;

		// We have three frames: encode them into one RGB PNG.
		uint8_t *encoded_buffer = nullptr;
		size_t buffer_len = 0;
		auto start_time = std::chrono::high_resolution_clock::now();
		try
		{
			encodePNG(group[0], group[1], group[2], encoded_buffer, buffer_len);
		}
		catch (std::exception const &e)
		{
			LOG_ERROR("PNG RGB encoding error: " << e.what());
			group.clear();
			continue;
		}
		...
		output_queue_[0].push(output_item);
		output_cond_var_.notify_one();
	}
}

void PngThreePhotoEncoder::outputThread()
{
	OutputItem item;
	uint64_t index = 0;
	while (true)
	...
	got_item:
		// Signal that we've finished with the input buffers. Each RGB PNG
		// consumes three input frames, so pop three CompletedRequests.
		for (unsigned int i = 0; i < item.frames_used; ++i)
